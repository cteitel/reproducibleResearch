...
First, deal with missing values in the dataset
...
par(mfrow=c(2,1),oma=c(2,2,3,0),mar=c(2,3,2,3))
for(i in c('weekday','weekend')){
    temp=activityNoNA[which(activityNoNA$weekend==i),]
    
    stepsPerInterval=tapply(temp$steps,temp$interval,
                            function(steps) mean(na.omit(steps)))
    intervals=unique(temp$interval)
    
    plot(stepsPerInterval~intervals,main=i,ylim=c(0,250),
         type='l',xlab='',ylab='')

}
mtext('Daily Activity Time Series',outer=T,side=3)
